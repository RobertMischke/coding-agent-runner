...
/// reason is a self-crash (<see cref="RunOutcome.Failed"/>), not a kill — without
...
public static class RunStatusClassifier
{
    ...
    public static RunOutcome Classify(int? exitCode, RunStopReason stopReason)
    {
        ...
        if (stopReason != RunStopReason.None) return RunOutcome.Stopped;
        return exitCode == 0 ? RunOutcome.Completed : RunOutcome.Failed;
    }
}
